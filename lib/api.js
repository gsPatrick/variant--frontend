...
export const auth = {
  ...
  // Mescla campos no usuário guardado (após editar o perfil).
  patchUser(partial) {
    if (typeof window === 'undefined') return;
    const current = this.getUser() || {};
    localStorage.setItem(STORAGE.user, JSON.stringify({ ...current, ...partial }));
  },
  me: () => request('/auth/me'),
...
export const plotsApi = {
  ...
  depths: (id) => request(`/plots/${id}/soil-analyses/depths`),
  listAnalyses: (id) => request(`/plots/${id}/soil-analyses`),
  removeAnalysis: (plotId, analysisId) => request(`/plots/${plotId}/soil-analyses/${analysisId}`, { method: 'DELETE' }),
  evolution: (id, nutriente, depth) =>
    request(`/plots/${id}/soil-analyses/evolution?nutriente=${encodeURIComponent(nutriente)}${depth ? `&depth=${encodeURIComponent(depth)}` : ''}`),
  radar: (id, year, depth) =>
    request(`/plots/${id}/soil-analyses/radar?year=${year}${depth ? `&depth=${encodeURIComponent(depth)}` : ''}`),
...
export const settingsApi = {
  getRadarIdeals: () => request('/settings/radar-ideals'),
  saveRadarIdeals: (ideals) => request('/settings/radar-ideals', { method: 'PUT', body: { ideals } }),
  getSocialLinks: () => request('/settings/social-links'),
  saveSocialLinks: (links) => request('/settings/social-links', { method: 'PUT', body: { links } }),
};
